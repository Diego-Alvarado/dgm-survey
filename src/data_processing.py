...
def load_df(file_names: dict, key: int) -> tuple[pd.DataFrame]:
    df_materials = pd.read_csv(f'./src/data/sample_cvae/{file_names[key]["materials"]}', sep='\t')
    df_materials['structure'] = [file_names[key]["materials"][:-4] + f'_{id}.png' for id in df_materials['index']]
    
    df_sequences = pd.read_csv(f'./src/data/sample_cvae/{file_names[key]["sequences"]}', sep='\t')
